...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  // OpenAPI spec endpoint — serves docs/openapi.yaml as JSON
  let openApiSpec: Record<string, unknown> | null = null;

  fastify.get('/api/v1/openapi.json', async (_request, reply) => {
    if (!openApiSpec) {
      // Resolve spec path: try CWD-relative first (production), then
      // walk up from CWD to find the docs/ directory (monorepo layouts).
      const candidates = [
      ...
      openApiSpec = parseYaml(yamlContent) as Record<string, unknown>;
    }
